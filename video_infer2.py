...
def main():
    ...
    model.load_state_dict(torch.load('./best_unet_lane_detection.pth', map_location=torch.device('cuda'))['model_state_dict'])
    model.eval()
    ...
    # Open input video
    input_video = cv2.VideoCapture('../test.mov')
    ...
    output_video = cv2.VideoWriter('../output/vid.mp4', fourcc, fps, (width, height))
